...
FONT_FILENAME :: "assets/fonts/PixelifySans-Regular.ttf"
...
main :: proc() {
	...
	dir_path_to_file_infos :: proc(path: string) -> []os.File_Info {
...

		{
			...
			if file_info.type != os.File_Type.Directory {
	...
	f, _ := os.open(ATLAS_ODIN_OUTPUT_PATH, os.O_WRONLY | os.O_CREATE | os.O_TRUNC, {.Read_User, .Write_User, .Read_Group, .Read_Other},)
	defer os.close(f)
